...
@Component({
  selector: 'app-new-demande-transitaire',
  templateUrl: './new-demande-transitaire.component.html',
  styleUrls: ['./new-demande-transitaire.component.css']
})
export class NewDemandeTransitaireComponent implements OnInit {
  zones : Array<any> =[
    {name: 'ANP', value:'ANP'},
  ...
  onFileChangedim(event: Event) {
    // @ts-ignore
    this.fileSourcedim = event.target.files[0];
    console.log(this.fileSourcedim);
  }
  ...
  onSubmit() {
    ...
    // @ts-ignore
    formData.append('dim', this.fileSourcedim);

    formData.append('TYPE', "TRANSITAIRE");
